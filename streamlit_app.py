...
with st.form('Données'):
    ...
    with st.expander("ℹ️ En savoir plus sur le risque de SRI"):
        st.write("Le syndrome de renutrition inappropriée ou SRI correspond à une augmentation trop rapide des ingestas (calories et micronutriments)  trop rapide et trop brusque chez un patient dénutris ou après un jeune prolongé. Ce syndrome peut être fatal pour le patient dû aux troubles  électrolytiques pouvant mené jusqu’à une défaillance multiviscérale. C’est pour cela qu’il est important de savoir le détecter le plus tôt possible.  Les possibles manifestations cliniques sont : une hypertension artérielle, des oedèmes, une insuffisance cardiaque transitoire…")
